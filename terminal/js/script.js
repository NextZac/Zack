...
var prephraseRoot = '<span class="red">root@arch</span>:<span class="blue">~</span># ';
var prephrase = '<span class="red">zack@arch</span>:<span class="blue">~</span># ';
...
{
	var res = '';
	if (command === './welcome.sh') res = 'Welcome to <span class="red">ZacK#3585\'\s Arch Terminal</span><br></span><br>Made By:</span><span class="red"> ZacK#3585</span><br><br><b>Type \'help\' for more details on the commands</b>'
	else
	{
...
$('form').on('submit', function(e) {
	...
	try {
			...
			if (vInput.startsWith("./")) {
				if (vInput === "./root/welcome.sh") {
					...
					launchCommandAsVisitor({
						cmd: vInput,
						res: '<div><p>bash: ' + vInput + ': No such file or directory</p></div>'
					});
				}
				...
				if (typeof loginArray[2] !== "undefined") {
					if (loginArray[2] === "visitor") {
						...
						res = 'bash: No passwd entry for user ' + loginArray[2] + '';
					}
					launchCommandAsVisitor({
				...
				if (vInput === "music play") {
					...
					var win = window.open("https://soundcloud.com/ryan-cc/weather", "_blank");
					win.focus();
					launchCommandAsVisitor({
						cmd: vInput,
						res: ''
				...
				neofetch({
						cmd: $('input').val(),
						res: '<div>zack@arch</span><br><span class="red">OS:</span> Arch Linux ' + OSVer + '<br><span class="red">Kernel:</span> ' + Kernel + '<br><span class="red">Uptime:</span> ' + upTime + '<br><span class="red">Shell:</span> ' + Shell + '<br><span class="red">Disk:</span> ' + Disk + '<br></p></div>'
					});
			} else if (vInput.trim() !== "") {
		...
		if (root > 0) {
			launchCommandAsRoot({
				cmd: $('input').val(),
				res: '<div><p>bash: ' + $('input').val() + ': command not found</p></div>'
			});
		} else {
			launchCommandAsVisitor({
				cmd: $('input').val(),
